---
title: " "
---

![Rob Moss standing outside in front of a tree](/images/profile-pic.jpg){ width="250" }
/// caption
I'm a Senior Research Fellow in the [Centre for Epidemiology and Biostatistics](https://epi.unimelb.edu.au/) at the
[Melbourne School of Population and Global Health](https://mspgh.unimelb.edu.au/), [University of Melbourne](https://www.unimelb.edu.au).

I'm also a member of Australia's [National Respiratory Infections Surveillance Committee](https://www.cdc.gov.au/about/advisory-groups-and-committees/national-respiratory-infections-surveillance-committee-nrisc).
///

## :ardis-pathogen: Research

[My research](projects.md) focuses on predicting and mitigating the burden of infectious disease epidemics, through the use of mathematical/computational modelling and Bayesian inference.
My primary focus is on seasonal and pandemic influenza, other respiratory pathogens such as RSV and COVID-19, and malaria.
This includes:

- Using **scenario modelling to inform recommendations** for effective health interventions; and
- Synthesising models and surveillance data to generate **near-real-time epidemic forecasts**.

I am a proponent of [Reproducible Research](https://en.wikipedia.org/wiki/Reproducibility), [Open Access](https://en.wikipedia.org/wiki/Open_access), and [Free and Open Source Software](https://en.wikipedia.org/wiki/Free_and_open-source_software), and produce open source software packages through my research activities.
See my [Software](software.md) page for details.

> *A model that cannot be explained should not inform decisions that require justification.*
>
> — [Because the model said so](https://doi.org/10.1016/j.ijforecast.2021.01.028)

## :ardis-teaching: Training

[![Git is my lab book](/images/logo-gimlb.png){ align=left width="64" }](https://git-is-my-lab-book.net/) I am a lead developer of <a href="https://git-is-my-lab-book.net/">Git is my lab book</a>, a collection of materials to support early- and mid-career researchers who want to advance their computing skills and make effective use of available tools and computing infrastructure.

## :ardis-info: Networks

<div class="icon-list" markdown>
[![ACEFA](/images/network-acefa.png "ACEFA"){ width=128 class="network-logo" }](https://acefa-epi-analytics.org/)
[![qMalaria](/images/network-qmalaria.png "qMalaria"){ width=50 class="network-logo" }](https://qmalaria.org/)
[![APPRISE](/images/network-apprise.png "APPRISE"){ width=128 class="network-logo" }](https://www.apprise.org.au/)
[![SPECTRUM](/images/network-spectrum.png "SPECTRUM"){ width=128 class="network-logo" }](https://www.spectrum.edu.au/)
[![SPARK](/images/network-spark.png "SPARK"){ width=128 class="network-logo" }](https://www.spark.edu.au/)
[![Flutracking](/images/network-flutracking.png "Flutracking"){ width=128 class="network-logo" }](https://info.flutracking.net/)
[![PRISM](/images/network-prism.jpg "PRISM"){ width=128 class="network-logo" }](https://www.prism.edu.au/)
[![RSE-AUNZ](/images/network-rse-aunz.png "RSE-AUNZ"){ width=49 class="network-logo" }](https://rse-aunz.github.io/)
</div>
